...
*/
  ...
  $('form.offers_item_add2cart').submit(function(){
    ...
    checkMinQunatityVal($('.add-to-cart-input-quantity', form));
...
//           btnSubmit.removeClass('btn-disabled');
  ...
  function checkMinQunatityVal(input){
    ...
    minVal = Number(input.data('inputmin'));
...
//     }
  ...
  $('.show-all-offers').click(function(){
    ...
    $('.cat-hidden', parent).removeClass('cat-hidden').addClass('cat-visible');
    $(this).hide();
...
//     var url = $(this).attr('href')+'&AJAX_SORT_OFFERS=Y';
...
//     BX.ajax.insertToNode(url, 'offersItems-block');
    
//     $.get($(this).attr('href'), {AJAX_SORT_OFFERS: 'Y'}, function(data){
